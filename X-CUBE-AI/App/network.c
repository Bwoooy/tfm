/**
  ******************************************************************************
  * @file    network.c
  * @author  AST Embedded Analytics Research Platform
  * @date    Sat Jun  3 21:00:36 2023
  * @brief   AI Tool Automatic Code Generator for Embedded NN computing
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2023 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  ******************************************************************************
  */


#include "network.h"
#include "network_data.h"

#include "ai_platform.h"
#include "ai_platform_interface.h"
#include "ai_math_helpers.h"

#include "core_common.h"
#include "core_convert.h"

#include "layers.h"



#undef AI_NET_OBJ_INSTANCE
#define AI_NET_OBJ_INSTANCE g_network
 
#undef AI_NETWORK_MODEL_SIGNATURE
#define AI_NETWORK_MODEL_SIGNATURE     "a623799814b86cf578eab29ad6ff8f87"

#ifndef AI_TOOLS_REVISION_ID
#define AI_TOOLS_REVISION_ID     ""
#endif

#undef AI_TOOLS_DATE_TIME
#define AI_TOOLS_DATE_TIME   "Sat Jun  3 21:00:36 2023"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_NETWORK_N_BATCHES
#define AI_NETWORK_N_BATCHES         (1)

static ai_ptr g_network_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_network_weights_map[1] = AI_C_ARRAY_INIT;



/**  Array declarations section  **********************************************/
/* Array#0 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_7_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#1 */
AI_ARRAY_OBJ_DECLARE(
  input_1_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
  NULL, NULL, 187, AI_STATIC)
/* Array#2 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_7_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#3 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5856, AI_STATIC)
/* Array#4 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_1_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5856, AI_STATIC)
/* Array#5 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_8_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#6 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_2_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5856, AI_STATIC)
/* Array#7 */
AI_ARRAY_OBJ_DECLARE(
  add_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5856, AI_STATIC)
/* Array#8 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_8_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#9 */
AI_ARRAY_OBJ_DECLARE(
  activation_3_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5856, AI_STATIC)
/* Array#10 */
AI_ARRAY_OBJ_DECLARE(
  max_pooling1d_1_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 2880, AI_STATIC)
/* Array#11 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_9_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#12 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_5_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 2880, AI_STATIC)
/* Array#13 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_6_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 2880, AI_STATIC)
/* Array#14 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_9_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#15 */
AI_ARRAY_OBJ_DECLARE(
  add_2_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 2880, AI_STATIC)
/* Array#16 */
AI_ARRAY_OBJ_DECLARE(
  activation_5_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 2880, AI_STATIC)
/* Array#17 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_10_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#18 */
AI_ARRAY_OBJ_DECLARE(
  max_pooling1d_2_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1376, AI_STATIC)
/* Array#19 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_7_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1376, AI_STATIC)
/* Array#20 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_10_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#21 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_8_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1376, AI_STATIC)
/* Array#22 */
AI_ARRAY_OBJ_DECLARE(
  add_3_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1376, AI_STATIC)
/* Array#23 */
AI_ARRAY_OBJ_DECLARE(
  dense_dense_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 8192, AI_STATIC)
/* Array#24 */
AI_ARRAY_OBJ_DECLARE(
  activation_7_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1376, AI_STATIC)
/* Array#25 */
AI_ARRAY_OBJ_DECLARE(
  max_pooling1d_3_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 640, AI_STATIC)
/* Array#26 */
AI_ARRAY_OBJ_DECLARE(
  dense_dense_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#27 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_9_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 640, AI_STATIC)
/* Array#28 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_10_conv2d_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 640, AI_STATIC)
/* Array#29 */
AI_ARRAY_OBJ_DECLARE(
  dense_1_dense_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1024, AI_STATIC)
/* Array#30 */
AI_ARRAY_OBJ_DECLARE(
  add_4_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 640, AI_STATIC)
/* Array#31 */
AI_ARRAY_OBJ_DECLARE(
  activation_9_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 640, AI_STATIC)
/* Array#32 */
AI_ARRAY_OBJ_DECLARE(
  dense_1_dense_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#33 */
AI_ARRAY_OBJ_DECLARE(
  max_pooling1d_4_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 256, AI_STATIC)
/* Array#34 */
AI_ARRAY_OBJ_DECLARE(
  dense_dense_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#35 */
AI_ARRAY_OBJ_DECLARE(
  dense_2_dense_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 160, AI_STATIC)
/* Array#36 */
AI_ARRAY_OBJ_DECLARE(
  activation_10_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#37 */
AI_ARRAY_OBJ_DECLARE(
  dense_1_dense_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#38 */
AI_ARRAY_OBJ_DECLARE(
  dense_2_dense_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5, AI_STATIC)
/* Array#39 */
AI_ARRAY_OBJ_DECLARE(
  dense_2_dense_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5, AI_STATIC)
/* Array#40 */
AI_ARRAY_OBJ_DECLARE(
  softmax_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
  NULL, NULL, 5, AI_STATIC)
/* Array#41 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 160, AI_STATIC)
/* Array#42 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#43 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_1_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#44 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_1_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#45 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_2_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#46 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_2_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#47 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_5_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#48 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_5_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/* Array#49 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_6_conv2d_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5120, AI_STATIC)
/* Array#50 */
AI_ARRAY_OBJ_DECLARE(
  conv1d_6_conv2d_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 32, AI_STATIC)
/**  Tensor declarations section  *********************************************/
/* Tensor #0 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_7_conv2d_weights, AI_STATIC,
  0, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_7_conv2d_weights_array, NULL)

/* Tensor #1 */
AI_TENSOR_OBJ_DECLARE(
  input_1_output, AI_STATIC,
  1, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 1, 187), AI_STRIDE_INIT(4, 4, 4, 4, 4),
  1, &input_1_output_array, NULL)

/* Tensor #2 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_7_conv2d_bias, AI_STATIC,
  2, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_7_conv2d_bias_array, NULL)

/* Tensor #3 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_conv2d_output, AI_STATIC,
  3, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 183), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_conv2d_output_array, NULL)

/* Tensor #4 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_1_conv2d_output, AI_STATIC,
  4, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 183), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_1_conv2d_output_array, NULL)

/* Tensor #5 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_8_conv2d_weights, AI_STATIC,
  5, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_8_conv2d_weights_array, NULL)

/* Tensor #6 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_2_conv2d_output, AI_STATIC,
  6, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 183), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_2_conv2d_output_array, NULL)

/* Tensor #7 */
AI_TENSOR_OBJ_DECLARE(
  add_output, AI_STATIC,
  7, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 183), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &add_output_array, NULL)

/* Tensor #8 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_8_conv2d_bias, AI_STATIC,
  8, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_8_conv2d_bias_array, NULL)

/* Tensor #9 */
AI_TENSOR_OBJ_DECLARE(
  activation_3_output, AI_STATIC,
  9, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 183), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &activation_3_output_array, NULL)

/* Tensor #10 */
AI_TENSOR_OBJ_DECLARE(
  max_pooling1d_1_output, AI_STATIC,
  10, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 90), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &max_pooling1d_1_output_array, NULL)

/* Tensor #11 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_9_conv2d_weights, AI_STATIC,
  11, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_9_conv2d_weights_array, NULL)

/* Tensor #12 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_5_conv2d_output, AI_STATIC,
  12, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 90), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_5_conv2d_output_array, NULL)

/* Tensor #13 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_6_conv2d_output, AI_STATIC,
  13, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 90), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_6_conv2d_output_array, NULL)

/* Tensor #14 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_9_conv2d_bias, AI_STATIC,
  14, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_9_conv2d_bias_array, NULL)

/* Tensor #15 */
AI_TENSOR_OBJ_DECLARE(
  add_2_output, AI_STATIC,
  15, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 90), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &add_2_output_array, NULL)

/* Tensor #16 */
AI_TENSOR_OBJ_DECLARE(
  activation_5_output, AI_STATIC,
  16, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 90), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &activation_5_output_array, NULL)

/* Tensor #17 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_10_conv2d_weights, AI_STATIC,
  17, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_10_conv2d_weights_array, NULL)

/* Tensor #18 */
AI_TENSOR_OBJ_DECLARE(
  max_pooling1d_2_output, AI_STATIC,
  18, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 43), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &max_pooling1d_2_output_array, NULL)

/* Tensor #19 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_7_conv2d_output, AI_STATIC,
  19, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 43), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_7_conv2d_output_array, NULL)

/* Tensor #20 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_10_conv2d_bias, AI_STATIC,
  20, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_10_conv2d_bias_array, NULL)

/* Tensor #21 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_8_conv2d_output, AI_STATIC,
  21, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 43), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_8_conv2d_output_array, NULL)

/* Tensor #22 */
AI_TENSOR_OBJ_DECLARE(
  add_3_output, AI_STATIC,
  22, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 43), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &add_3_output_array, NULL)

/* Tensor #23 */
AI_TENSOR_OBJ_DECLARE(
  dense_dense_weights, AI_STATIC,
  23, 0x0,
  AI_SHAPE_INIT(4, 256, 32, 1, 1), AI_STRIDE_INIT(4, 4, 1024, 32768, 32768),
  1, &dense_dense_weights_array, NULL)

/* Tensor #24 */
AI_TENSOR_OBJ_DECLARE(
  activation_7_output, AI_STATIC,
  24, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 43), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &activation_7_output_array, NULL)

/* Tensor #25 */
AI_TENSOR_OBJ_DECLARE(
  max_pooling1d_3_output, AI_STATIC,
  25, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 20), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &max_pooling1d_3_output_array, NULL)

/* Tensor #26 */
AI_TENSOR_OBJ_DECLARE(
  dense_dense_bias, AI_STATIC,
  26, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &dense_dense_bias_array, NULL)

/* Tensor #27 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_9_conv2d_output, AI_STATIC,
  27, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 20), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_9_conv2d_output_array, NULL)

/* Tensor #28 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_10_conv2d_output, AI_STATIC,
  28, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 20), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_10_conv2d_output_array, NULL)

/* Tensor #29 */
AI_TENSOR_OBJ_DECLARE(
  dense_1_dense_weights, AI_STATIC,
  29, 0x0,
  AI_SHAPE_INIT(4, 32, 32, 1, 1), AI_STRIDE_INIT(4, 4, 128, 4096, 4096),
  1, &dense_1_dense_weights_array, NULL)

/* Tensor #30 */
AI_TENSOR_OBJ_DECLARE(
  add_4_output, AI_STATIC,
  30, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 20), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &add_4_output_array, NULL)

/* Tensor #31 */
AI_TENSOR_OBJ_DECLARE(
  activation_9_output, AI_STATIC,
  31, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 20), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &activation_9_output_array, NULL)

/* Tensor #32 */
AI_TENSOR_OBJ_DECLARE(
  dense_1_dense_bias, AI_STATIC,
  32, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &dense_1_dense_bias_array, NULL)

/* Tensor #33 */
AI_TENSOR_OBJ_DECLARE(
  max_pooling1d_4_output, AI_STATIC,
  33, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 8), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &max_pooling1d_4_output_array, NULL)

/* Tensor #34 */
AI_TENSOR_OBJ_DECLARE(
  max_pooling1d_4_output0, AI_STATIC,
  34, 0x0,
  AI_SHAPE_INIT(4, 1, 256, 1, 1), AI_STRIDE_INIT(4, 4, 4, 1024, 1024),
  1, &max_pooling1d_4_output_array, NULL)

/* Tensor #35 */
AI_TENSOR_OBJ_DECLARE(
  dense_dense_output, AI_STATIC,
  35, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &dense_dense_output_array, NULL)

/* Tensor #36 */
AI_TENSOR_OBJ_DECLARE(
  dense_2_dense_weights, AI_STATIC,
  36, 0x0,
  AI_SHAPE_INIT(4, 32, 5, 1, 1), AI_STRIDE_INIT(4, 4, 128, 640, 640),
  1, &dense_2_dense_weights_array, NULL)

/* Tensor #37 */
AI_TENSOR_OBJ_DECLARE(
  activation_10_output, AI_STATIC,
  37, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &activation_10_output_array, NULL)

/* Tensor #38 */
AI_TENSOR_OBJ_DECLARE(
  dense_1_dense_output, AI_STATIC,
  38, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &dense_1_dense_output_array, NULL)

/* Tensor #39 */
AI_TENSOR_OBJ_DECLARE(
  dense_2_dense_bias, AI_STATIC,
  39, 0x0,
  AI_SHAPE_INIT(4, 1, 5, 1, 1), AI_STRIDE_INIT(4, 4, 4, 20, 20),
  1, &dense_2_dense_bias_array, NULL)

/* Tensor #40 */
AI_TENSOR_OBJ_DECLARE(
  dense_2_dense_output, AI_STATIC,
  40, 0x0,
  AI_SHAPE_INIT(4, 1, 5, 1, 1), AI_STRIDE_INIT(4, 4, 4, 20, 20),
  1, &dense_2_dense_output_array, NULL)

/* Tensor #41 */
AI_TENSOR_OBJ_DECLARE(
  softmax_output, AI_STATIC,
  41, 0x0,
  AI_SHAPE_INIT(4, 1, 5, 1, 1), AI_STRIDE_INIT(4, 4, 4, 20, 20),
  1, &softmax_output_array, NULL)

/* Tensor #42 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_conv2d_weights, AI_STATIC,
  42, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 5, 32), AI_STRIDE_INIT(4, 4, 4, 4, 20),
  1, &conv1d_conv2d_weights_array, NULL)

/* Tensor #43 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_conv2d_bias, AI_STATIC,
  43, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_conv2d_bias_array, NULL)

/* Tensor #44 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_1_conv2d_weights, AI_STATIC,
  44, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_1_conv2d_weights_array, NULL)

/* Tensor #45 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_1_conv2d_bias, AI_STATIC,
  45, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_1_conv2d_bias_array, NULL)

/* Tensor #46 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_2_conv2d_weights, AI_STATIC,
  46, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_2_conv2d_weights_array, NULL)

/* Tensor #47 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_2_conv2d_bias, AI_STATIC,
  47, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_2_conv2d_bias_array, NULL)

/* Tensor #48 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_5_conv2d_weights, AI_STATIC,
  48, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_5_conv2d_weights_array, NULL)

/* Tensor #49 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_5_conv2d_bias, AI_STATIC,
  49, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_5_conv2d_bias_array, NULL)

/* Tensor #50 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_6_conv2d_weights, AI_STATIC,
  50, 0x0,
  AI_SHAPE_INIT(4, 32, 1, 5, 32), AI_STRIDE_INIT(4, 4, 128, 128, 640),
  1, &conv1d_6_conv2d_weights_array, NULL)

/* Tensor #51 */
AI_TENSOR_OBJ_DECLARE(
  conv1d_6_conv2d_bias, AI_STATIC,
  51, 0x0,
  AI_SHAPE_INIT(4, 1, 32, 1, 1), AI_STRIDE_INIT(4, 4, 4, 128, 128),
  1, &conv1d_6_conv2d_bias_array, NULL)



/**  Layer declarations section  **********************************************/


AI_TENSOR_CHAIN_OBJ_DECLARE(
  softmax_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &dense_2_dense_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &softmax_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  softmax_layer, 31,
  NL_TYPE, 0x0, NULL,
  nl, forward_sm,
  &softmax_chain,
  NULL, &softmax_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  dense_2_dense_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &dense_1_dense_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &dense_2_dense_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &dense_2_dense_weights, &dense_2_dense_bias),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  dense_2_dense_layer, 30,
  DENSE_TYPE, 0x0, NULL,
  dense, forward_dense,
  &dense_2_dense_chain,
  NULL, &softmax_layer, AI_STATIC, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  dense_1_dense_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_10_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &dense_1_dense_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &dense_1_dense_weights, &dense_1_dense_bias),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  dense_1_dense_layer, 29,
  DENSE_TYPE, 0x0, NULL,
  dense, forward_dense,
  &dense_1_dense_chain,
  NULL, &dense_2_dense_layer, AI_STATIC, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  activation_10_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &dense_dense_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_10_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  activation_10_layer, 28,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &activation_10_chain,
  NULL, &dense_1_dense_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  dense_dense_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_4_output0),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &dense_dense_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &dense_dense_weights, &dense_dense_bias),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  dense_dense_layer, 27,
  DENSE_TYPE, 0x0, NULL,
  dense, forward_dense,
  &dense_dense_chain,
  NULL, &activation_10_layer, AI_STATIC, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  max_pooling1d_4_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_9_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_4_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  max_pooling1d_4_layer, 25,
  POOL_TYPE, 0x0, NULL,
  pool, forward_mp,
  &max_pooling1d_4_chain,
  NULL, &dense_dense_layer, AI_STATIC, 
  .pool_size = AI_SHAPE_2D_INIT(1, 5), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  activation_9_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_4_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_9_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  activation_9_layer, 24,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &activation_9_chain,
  NULL, &max_pooling1d_4_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  add_4_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &conv1d_10_conv2d_output, &max_pooling1d_3_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_4_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  add_4_layer, 23,
  ELTWISE_TYPE, 0x0, NULL,
  eltwise, forward_eltwise,
  &add_4_chain,
  NULL, &activation_9_layer, AI_STATIC, 
  .operation = ai_sum_f32, 
  .buffer_operation = ai_sum_buffer_f32, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_10_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_9_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_10_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_10_conv2d_weights, &conv1d_10_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_10_conv2d_layer, 22,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_10_conv2d_chain,
  NULL, &add_4_layer, AI_STATIC, 
  .groups = 1, 
  .nl_func = NULL, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_9_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_3_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_9_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_9_conv2d_weights, &conv1d_9_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_9_conv2d_layer, 21,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_9_conv2d_chain,
  NULL, &conv1d_10_conv2d_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  max_pooling1d_3_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_7_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_3_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  max_pooling1d_3_layer, 19,
  POOL_TYPE, 0x0, NULL,
  pool, forward_mp,
  &max_pooling1d_3_chain,
  NULL, &conv1d_9_conv2d_layer, AI_STATIC, 
  .pool_size = AI_SHAPE_2D_INIT(1, 5), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  activation_7_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_3_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_7_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  activation_7_layer, 18,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &activation_7_chain,
  NULL, &max_pooling1d_3_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  add_3_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &conv1d_8_conv2d_output, &max_pooling1d_2_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_3_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  add_3_layer, 17,
  ELTWISE_TYPE, 0x0, NULL,
  eltwise, forward_eltwise,
  &add_3_chain,
  NULL, &activation_7_layer, AI_STATIC, 
  .operation = ai_sum_f32, 
  .buffer_operation = ai_sum_buffer_f32, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_8_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_7_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_8_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_8_conv2d_weights, &conv1d_8_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_8_conv2d_layer, 16,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_8_conv2d_chain,
  NULL, &add_3_layer, AI_STATIC, 
  .groups = 1, 
  .nl_func = NULL, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_7_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_2_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_7_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_7_conv2d_weights, &conv1d_7_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_7_conv2d_layer, 15,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_7_conv2d_chain,
  NULL, &conv1d_8_conv2d_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  max_pooling1d_2_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_5_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_2_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  max_pooling1d_2_layer, 13,
  POOL_TYPE, 0x0, NULL,
  pool, forward_mp,
  &max_pooling1d_2_chain,
  NULL, &conv1d_7_conv2d_layer, AI_STATIC, 
  .pool_size = AI_SHAPE_2D_INIT(1, 5), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  activation_5_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_2_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_5_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  activation_5_layer, 12,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &activation_5_chain,
  NULL, &max_pooling1d_2_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  add_2_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &conv1d_6_conv2d_output, &max_pooling1d_1_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_2_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  add_2_layer, 11,
  ELTWISE_TYPE, 0x0, NULL,
  eltwise, forward_eltwise,
  &add_2_chain,
  NULL, &activation_5_layer, AI_STATIC, 
  .operation = ai_sum_f32, 
  .buffer_operation = ai_sum_buffer_f32, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_6_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_5_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_6_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_6_conv2d_weights, &conv1d_6_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_6_conv2d_layer, 10,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_6_conv2d_chain,
  NULL, &add_2_layer, AI_STATIC, 
  .groups = 1, 
  .nl_func = NULL, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_5_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_1_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_5_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_5_conv2d_weights, &conv1d_5_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_5_conv2d_layer, 9,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_5_conv2d_chain,
  NULL, &conv1d_6_conv2d_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  max_pooling1d_1_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_3_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &max_pooling1d_1_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  max_pooling1d_1_layer, 7,
  POOL_TYPE, 0x0, NULL,
  pool, forward_mp,
  &max_pooling1d_1_chain,
  NULL, &conv1d_5_conv2d_layer, AI_STATIC, 
  .pool_size = AI_SHAPE_2D_INIT(1, 5), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  activation_3_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &activation_3_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  activation_3_layer, 6,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &activation_3_chain,
  NULL, &max_pooling1d_1_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  add_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &conv1d_2_conv2d_output, &conv1d_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &add_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  add_layer, 5,
  ELTWISE_TYPE, 0x0, NULL,
  eltwise, forward_eltwise,
  &add_chain,
  NULL, &activation_3_layer, AI_STATIC, 
  .operation = ai_sum_f32, 
  .buffer_operation = ai_sum_buffer_f32, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_2_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_1_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_2_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_2_conv2d_weights, &conv1d_2_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_2_conv2d_layer, 4,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_2_conv2d_chain,
  NULL, &add_layer, AI_STATIC, 
  .groups = 1, 
  .nl_func = NULL, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_1_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_1_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_1_conv2d_weights, &conv1d_1_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_1_conv2d_layer, 3,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_1_conv2d_chain,
  NULL, &conv1d_2_conv2d_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = nl_func_relu_array_f32, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 2, 0, 2, 0), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  conv1d_conv2d_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &input_1_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &conv1d_conv2d_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &conv1d_conv2d_weights, &conv1d_conv2d_bias, NULL),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  conv1d_conv2d_layer, 1,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d,
  &conv1d_conv2d_chain,
  NULL, &conv1d_1_conv2d_layer, AI_STATIC, 
  .groups = 1, 
  .nl_func = NULL, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
)


#if (AI_TOOLS_API_VERSION < AI_TOOLS_API_VERSION_1_5)

AI_NETWORK_OBJ_DECLARE(
  AI_NET_OBJ_INSTANCE, AI_STATIC,
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 203412, 1, 1),
    203412, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 47360, 1, 1),
    47360, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_IN_NUM, &input_1_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_OUT_NUM, &softmax_output),
  &conv1d_conv2d_layer, 0, NULL)

#else

AI_NETWORK_OBJ_DECLARE(
  AI_NET_OBJ_INSTANCE, AI_STATIC,
  AI_BUFFER_ARRAY_OBJ_INIT_STATIC(
  	AI_FLAG_NONE, 1,
    AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
      AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 203412, 1, 1),
      203412, NULL, NULL)
  ),
  AI_BUFFER_ARRAY_OBJ_INIT_STATIC(
  	AI_FLAG_NONE, 1,
    AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
      AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 47360, 1, 1),
      47360, NULL, NULL)
  ),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_IN_NUM, &input_1_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_OUT_NUM, &softmax_output),
  &conv1d_conv2d_layer, 0, NULL)

#endif	/*(AI_TOOLS_API_VERSION < AI_TOOLS_API_VERSION_1_5)*/


/******************************************************************************/
AI_DECLARE_STATIC
ai_bool network_configure_activations(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_activations_map(g_network_activations_map, 1, params)) {
    /* Updating activations (byte) offsets */
    
    conv1d_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 23936);
    conv1d_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 23936);
    
    conv1d_1_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 512);
    conv1d_1_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 512);
    
    conv1d_2_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    conv1d_2_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    add_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    add_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    activation_3_output_array.data = AI_PTR(g_network_activations_map[0] + 23424);
    activation_3_output_array.data_start = AI_PTR(g_network_activations_map[0] + 23424);
    
    max_pooling1d_1_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    max_pooling1d_1_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    conv1d_5_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 11520);
    conv1d_5_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 11520);
    
    conv1d_6_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 23040);
    conv1d_6_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 23040);
    
    add_2_output_array.data = AI_PTR(g_network_activations_map[0] + 11520);
    add_2_output_array.data_start = AI_PTR(g_network_activations_map[0] + 11520);
    
    activation_5_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    activation_5_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    max_pooling1d_2_output_array.data = AI_PTR(g_network_activations_map[0] + 11520);
    max_pooling1d_2_output_array.data_start = AI_PTR(g_network_activations_map[0] + 11520);
    
    conv1d_7_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    conv1d_7_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    conv1d_8_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 5504);
    conv1d_8_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 5504);
    
    add_3_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    add_3_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    activation_7_output_array.data = AI_PTR(g_network_activations_map[0] + 5504);
    activation_7_output_array.data_start = AI_PTR(g_network_activations_map[0] + 5504);
    
    max_pooling1d_3_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    max_pooling1d_3_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    conv1d_9_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 2560);
    conv1d_9_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 2560);
    
    conv1d_10_conv2d_output_array.data = AI_PTR(g_network_activations_map[0] + 5120);
    conv1d_10_conv2d_output_array.data_start = AI_PTR(g_network_activations_map[0] + 5120);
    
    add_4_output_array.data = AI_PTR(g_network_activations_map[0] + 2560);
    add_4_output_array.data_start = AI_PTR(g_network_activations_map[0] + 2560);
    
    activation_9_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    activation_9_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    max_pooling1d_4_output_array.data = AI_PTR(g_network_activations_map[0] + 2560);
    max_pooling1d_4_output_array.data_start = AI_PTR(g_network_activations_map[0] + 2560);
    
    dense_dense_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    dense_dense_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    activation_10_output_array.data = AI_PTR(g_network_activations_map[0] + 128);
    activation_10_output_array.data_start = AI_PTR(g_network_activations_map[0] + 128);
    
    dense_1_dense_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    dense_1_dense_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    
    dense_2_dense_output_array.data = AI_PTR(g_network_activations_map[0] + 128);
    dense_2_dense_output_array.data_start = AI_PTR(g_network_activations_map[0] + 128);
    
    return true;
  }
  AI_ERROR_TRAP(net_ctx, INIT_FAILED, NETWORK_ACTIVATIONS);
  return false;
}



/******************************************************************************/
AI_DECLARE_STATIC
ai_bool network_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_weights_map(g_network_weights_map, 1, params)) {
    /* Updating weights (byte) offsets */
    
    conv1d_7_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_7_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 0);
    conv1d_7_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 0);
    
    conv1d_7_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_7_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 20480);
    conv1d_7_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 20480);
    
    conv1d_8_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_8_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 20608);
    conv1d_8_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 20608);
    
    conv1d_8_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_8_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 41088);
    conv1d_8_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 41088);
    
    conv1d_9_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_9_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 41216);
    conv1d_9_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 41216);
    
    conv1d_9_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_9_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 61696);
    conv1d_9_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 61696);
    
    conv1d_10_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_10_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 61824);
    conv1d_10_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 61824);
    
    conv1d_10_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_10_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 82304);
    conv1d_10_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 82304);
    
    dense_dense_weights_array.format |= AI_FMT_FLAG_CONST;
    dense_dense_weights_array.data = AI_PTR(g_network_weights_map[0] + 82432);
    dense_dense_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 82432);
    
    dense_dense_bias_array.format |= AI_FMT_FLAG_CONST;
    dense_dense_bias_array.data = AI_PTR(g_network_weights_map[0] + 115200);
    dense_dense_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 115200);
    
    dense_1_dense_weights_array.format |= AI_FMT_FLAG_CONST;
    dense_1_dense_weights_array.data = AI_PTR(g_network_weights_map[0] + 115328);
    dense_1_dense_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 115328);
    
    dense_1_dense_bias_array.format |= AI_FMT_FLAG_CONST;
    dense_1_dense_bias_array.data = AI_PTR(g_network_weights_map[0] + 119424);
    dense_1_dense_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 119424);
    
    dense_2_dense_weights_array.format |= AI_FMT_FLAG_CONST;
    dense_2_dense_weights_array.data = AI_PTR(g_network_weights_map[0] + 119552);
    dense_2_dense_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 119552);
    
    dense_2_dense_bias_array.format |= AI_FMT_FLAG_CONST;
    dense_2_dense_bias_array.data = AI_PTR(g_network_weights_map[0] + 120192);
    dense_2_dense_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 120192);
    
    conv1d_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 120212);
    conv1d_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 120212);
    
    conv1d_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 120852);
    conv1d_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 120852);
    
    conv1d_1_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_1_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 120980);
    conv1d_1_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 120980);
    
    conv1d_1_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_1_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 141460);
    conv1d_1_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 141460);
    
    conv1d_2_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_2_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 141588);
    conv1d_2_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 141588);
    
    conv1d_2_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_2_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 162068);
    conv1d_2_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 162068);
    
    conv1d_5_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_5_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 162196);
    conv1d_5_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 162196);
    
    conv1d_5_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_5_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 182676);
    conv1d_5_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 182676);
    
    conv1d_6_conv2d_weights_array.format |= AI_FMT_FLAG_CONST;
    conv1d_6_conv2d_weights_array.data = AI_PTR(g_network_weights_map[0] + 182804);
    conv1d_6_conv2d_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 182804);
    
    conv1d_6_conv2d_bias_array.format |= AI_FMT_FLAG_CONST;
    conv1d_6_conv2d_bias_array.data = AI_PTR(g_network_weights_map[0] + 203284);
    conv1d_6_conv2d_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 203284);
    
    return true;
  }
  AI_ERROR_TRAP(net_ctx, INIT_FAILED, NETWORK_WEIGHTS);
  return false;
}


/**  PUBLIC APIs SECTION  *****************************************************/


AI_DEPRECATED
AI_API_ENTRY
ai_bool ai_network_get_info(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

  if (report && net_ctx)
  {
    ai_network_report r = {
      .model_name        = AI_NETWORK_MODEL_NAME,
      .model_signature   = AI_NETWORK_MODEL_SIGNATURE,
      .model_datetime    = AI_TOOLS_DATE_TIME,
      
      .compile_datetime  = AI_TOOLS_COMPILE_TIME,
      
      .runtime_revision  = ai_platform_runtime_get_revision(),
      .runtime_version   = ai_platform_runtime_get_version(),

      .tool_revision     = AI_TOOLS_REVISION_ID,
      .tool_version      = {AI_TOOLS_VERSION_MAJOR, AI_TOOLS_VERSION_MINOR,
                            AI_TOOLS_VERSION_MICRO, 0x0},
      .tool_api_version  = AI_STRUCT_INIT,

      .api_version            = ai_platform_api_get_version(),
      .interface_api_version  = ai_platform_interface_api_get_version(),
      
      .n_macc            = 3537776,
      .n_inputs          = 0,
      .inputs            = NULL,
      .n_outputs         = 0,
      .outputs           = NULL,
      .params            = AI_STRUCT_INIT,
      .activations       = AI_STRUCT_INIT,
      .n_nodes           = 0,
      .signature         = 0x0,
    };

    if (!ai_platform_api_get_network_report(network, &r)) return false;

    *report = r;
    return true;
  }
  return false;
}


AI_API_ENTRY
ai_bool ai_network_get_report(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

  if (report && net_ctx)
  {
    ai_network_report r = {
      .model_name        = AI_NETWORK_MODEL_NAME,
      .model_signature   = AI_NETWORK_MODEL_SIGNATURE,
      .model_datetime    = AI_TOOLS_DATE_TIME,
      
      .compile_datetime  = AI_TOOLS_COMPILE_TIME,
      
      .runtime_revision  = ai_platform_runtime_get_revision(),
      .runtime_version   = ai_platform_runtime_get_version(),

      .tool_revision     = AI_TOOLS_REVISION_ID,
      .tool_version      = {AI_TOOLS_VERSION_MAJOR, AI_TOOLS_VERSION_MINOR,
                            AI_TOOLS_VERSION_MICRO, 0x0},
      .tool_api_version  = AI_STRUCT_INIT,

      .api_version            = ai_platform_api_get_version(),
      .interface_api_version  = ai_platform_interface_api_get_version(),
      
      .n_macc            = 3537776,
      .n_inputs          = 0,
      .inputs            = NULL,
      .n_outputs         = 0,
      .outputs           = NULL,
      .map_signature     = AI_MAGIC_SIGNATURE,
      .map_weights       = AI_STRUCT_INIT,
      .map_activations   = AI_STRUCT_INIT,
      .n_nodes           = 0,
      .signature         = 0x0,
    };

    if (!ai_platform_api_get_network_report(network, &r)) return false;

    *report = r;
    return true;
  }
  return false;
}

AI_API_ENTRY
ai_error ai_network_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_network_create(
  ai_handle* network, const ai_buffer* network_config)
{
  return ai_platform_network_create(
    network, network_config, 
    &AI_NET_OBJ_INSTANCE,
    AI_TOOLS_API_VERSION_MAJOR, AI_TOOLS_API_VERSION_MINOR, AI_TOOLS_API_VERSION_MICRO);
}

AI_API_ENTRY
ai_error ai_network_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
    ai_error err;
    ai_network_params params;

    err = ai_network_create(network, AI_NETWORK_DATA_CONFIG);
    if (err.type != AI_ERROR_NONE)
        return err;
    if (ai_network_data_params_get(&params) != true) {
        err = ai_network_get_error(*network);
        return err;
    }
#if defined(AI_NETWORK_DATA_ACTIVATIONS_COUNT)
    if (activations) {
        /* set the addresses of the activations buffers */
        for (int idx=0;idx<params.map_activations.size;idx++)
            AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
    }
#endif
#if defined(AI_NETWORK_DATA_WEIGHTS_COUNT)
    if (weights) {
        /* set the addresses of the weight buffers */
        for (int idx=0;idx<params.map_weights.size;idx++)
            AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
    }
#endif
    if (ai_network_init(*network, &params) != true) {
        err = ai_network_get_error(*network);
    }
    return err;
}

AI_API_ENTRY
ai_buffer* ai_network_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    ((ai_network *)network)->magic = AI_MAGIC_CONTEXT_TOKEN;
  }
  return ai_platform_inputs_get(network, n_buffer);
}

AI_API_ENTRY
ai_buffer* ai_network_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    ((ai_network *)network)->magic = AI_MAGIC_CONTEXT_TOKEN;
  }
  return ai_platform_outputs_get(network, n_buffer);
}

AI_API_ENTRY
ai_handle ai_network_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_network_init(
  ai_handle network, const ai_network_params* params)
{
  ai_network* net_ctx = ai_platform_network_init(network, params);
  if (!net_ctx) return false;

  ai_bool ok = true;
  ok &= network_configure_weights(net_ctx, params);
  ok &= network_configure_activations(net_ctx, params);

  ok &= ai_platform_network_post_init(network);

  return ok;
}


AI_API_ENTRY
ai_i32 ai_network_run(
  ai_handle network, const ai_buffer* input, ai_buffer* output)
{
  return ai_platform_network_process(network, input, output);
}

AI_API_ENTRY
ai_i32 ai_network_forward(ai_handle network, const ai_buffer* input)
{
  return ai_platform_network_process(network, input, NULL);
}



#undef AI_NETWORK_MODEL_SIGNATURE
#undef AI_NET_OBJ_INSTANCE
#undef AI_TOOLS_DATE_TIME
#undef AI_TOOLS_COMPILE_TIME

